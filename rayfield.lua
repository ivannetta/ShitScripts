...
local function getAssetUri(id: any): string
	-- Handle Lucide icons
	if type(id) == "string" and Icons then
		-- Only try to get Lucide icon if Icons library exists and id is a simple string (not URL)
		local success, icon = pcall(getIcon, id)
		if success then
			return "rbxassetid://" .. icon.id
		end
	end

	-- Handle regular asset IDs
	if type(id) == "number" then
		return "rbxassetid://" .. id
	end

	warn("Rayfield | Invalid asset format. Expected URL, icon name, or asset ID. Got: " .. tostring(id))
	return "rbxassetid://0"
end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		-- ImageLabel
		function Tab:CreateImageLabel(Settings: table)
			local ImageLabelValue = {}

			local ImageLabel = Elements.Template.Label:Clone()
			ImageLabel.Title:Destroy() -- Remove title
			if ImageLabel:FindFirstChild("ElementIndicator") then
				ImageLabel.ElementIndicator:Destroy() -- Remove element indicator if it exists
			end
			...
			Image.AnchorPoint = Vector2.new(0.5, 0.5) -- Center anchor point
			Image.Position = UDim2.new(0.5, 0, 0.5, 0) -- Center position
			Image.Size = UDim2.new(1, -20, 1, -20) -- Fill container with small padding
			Image.Image = typeof(Settings.Image) == "string" and Icons and "rbxassetid://" .. getIcon(Settings.Image).id
				or getAssetUri(Settings.Image)
			Image.ScaleType = Settings.ScaleType or Enum.ScaleType.Stretch
			Image.ResampleMode = Enum.ResamplerMode.Pixelated

			-- Apply rect offset/size if using Lucide icon
			if typeof(Settings.Image) == "string" and Icons then
				local asset = getIcon(Settings.Image)
				Image.ImageRectOffset = asset.imageRectOffset
				Image.ImageRectSize = asset.imageRectSize
			end
			ImageLabel.Visible = true
			ImageLabel.Parent = TabPage

			ImageLabel.Size = UDim2.new(1, -10, 0, Settings.Height or 120)
			...
			Image.Image = typeof(Settings.Image) == "string" and Icons and "rbxassetid://" .. getIcon(Settings.Image).id
				or getAssetUri(Settings.Image)
			Image.ScaleType = Settings.ScaleType or Enum.ScaleType.Stretch -- Use Stretch instead of Fit for full coverage
			Image.ResampleMode = Enum.ResamplerMode.Pixelated -- Better quality scaling
			...
			ImageLabel.BackgroundColor3 = Settings.Color or SelectedTheme.SecondaryElementBackground
			ImageLabel.UIStroke.Color = Settings.Color or SelectedTheme.SecondaryElementStroke

			-- Initial transparency
			ImageLabel.BackgroundTransparency = 1
			ImageLabel.UIStroke.Transparency = 1
			...
			TweenService:Create(
				ImageLabel,
				TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
				{ BackgroundTransparency = Settings.Color and 0.8 or 0 }
			):Play()
			TweenService:Create(
				ImageLabel.UIStroke,
				TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
				{ Transparency = Settings.Color and 0.7 or 0 }
			):Play()
			TweenService:Create(
				Image,
				TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
				{ ImageTransparency = Settings.Color and 0.2 or 0 }
			):Play()

			function ImageLabelValue:Set(NewImage, NewScaleType)
				if typeof(NewImage) == "string" and Icons then
					local asset = getIcon(NewImage)
					Image.Image = "rbxassetid://" .. asset.id
					Image.ImageRectOffset = asset.imageRectOffset
					Image.ImageRectSize = asset.imageRectSize
				else
					Image.Image = getAssetUri(NewImage)
					Image.ImageRectOffset = Vector2.new(0, 0)
					Image.ImageRectSize = Vector2.new(0, 0)
				end

				if NewScaleType then
					Image.ScaleType = NewScaleType
				end
			end

			Rayfield.Main:GetPropertyChangedSignal("BackgroundColor3"):Connect(function()
				ImageLabel.BackgroundColor3 = Settings.IgnoreTheme and (Settings.Color or ImageLabel.BackgroundColor3)
					or SelectedTheme.SecondaryElementBackground
				ImageLabel.UIStroke.Color = Settings.IgnoreTheme and (Settings.Color or ImageLabel.UIStroke.Color)
					or SelectedTheme.SecondaryElementStroke
			end)

			return ImageLabelValue
		end
...
if not useStudio then
	task.spawn(
		loadWithTimeout,
		"https://raw.githubusercontent.com/SiriusSoftwareLtd/Sirius/refs/heads/request/boost.lua"
	)
end
